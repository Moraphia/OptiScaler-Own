...
// Own UI: convert wide string literals to UTF-8 for Dear ImGui.
...
void RenderMenu(Config* config, float menuResScale)
{
    ...
    ImGui::Spacing();
    if (auto ch = ScopedCollapsingHeader(UI_CN("DLSS 神经渲染")); ch.IsHeaderOpen())
    {
        ScopedIndent indent {};
        ImGui::Spacing();

        bool enabled = config->DlssNrEnabled.value_or_default();
        if (ImGui::Checkbox(UI_CN("启用神经渲染"), &enabled))
            config->DlssNrEnabled = enabled;

        HelpMarker(UiUtf8(L"在帧生成处理之前，对超分输出进一步合成细节。\n\n本分支游戏包包含：\n  nvngx_dlssnr.dll       社区提供的 310.8 运行库（签名校验不通过）\n  nvngx.dll_dlssnr.dll   本项目编译的转发器\n\n该功能没有 NVIDIA 官方集成文档，目前属于直接调用的实验性实现，不受官方支持。").c_str());
        ...
        ImGui::TextDisabled(UI_CN("可绑定快捷键切换——请在“快捷键”中设置“神经渲染”。"));
        ...
        if (!enabled)
        {
            ImGui::TextDisabled(UI_CN("已关闭。模型仍保留在显存中，再次开启可立即生效。"));
        }
        else if (!DlssNr::IsRunning() && !vulkan)
        {
            ...
            if (reason[0] != 0)
            {
                ImGui::TextColored(ImVec4(1.0f, 0.4f, 0.35f, 1.0f), UI_CN("本次会话已停用：%s。"), reason);
                ImGui::SameLine();

                if (ImGui::SmallButton(UI_CN("重试")))
                     ...
                     feature != nullptr && feature->Api() == API::DX11 && !feature->IsWithDx12())
            {
                ImGui::TextColored(ImVec4(1.0f, 0.75f, 0.3f, 1.0f),
                                   UI_CN("%s 当前以原生 D3D11 运行，神经渲染模型不支持此路径。"),
                                   feature->Name().c_str());
                ImGui::TextDisabled(UI_CN("请选择上方带 w/Dx12 标记的超分器，然后重启游戏。"));
            }
            else if (enabled)
                ImGui::TextUnformatted(UI_CN("正在等待超分器开始运行……"));
        }
        else
        {
            ...
            if (ms.has_value())
                ImGui::TextColored(ImVec4(0.4f, 0.9f, 0.5f, 1.0f), UI_CN("运行中%s - 每帧 %.2f ms"),
                                   vulkan ? UI_CN("（Vulkan 原生）") : "", ms.value());
            else if (vulkan)
                // Measured but not yet read: the first few frames are still in the query ring.
                ImGui::TextColored(ImVec4(0.4f, 0.9f, 0.5f, 1.0f), UI_CN("Vulkan 原生运行中 - %llu 帧"),
                                   DlssNr::FramesVk());
            else
                ImGui::TextColored(ImVec4(0.4f, 0.9f, 0.5f, 1.0f), UI_CN("运行中。"));

            ImGui::SameLine();
            ImGui::TextDisabled("(?)");
            if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
                ImGui::SetTooltip("%s", UiUtf8(L"这里统计的是整个神经渲染流程，包括中间拷贝、Resolve 和模型本身。只统计模型会低估实际开销。\n\n可与窗口底部的帧时间对比，判断该功能实际增加了多少 GPU 开销。").c_str());
        }

        ImGui::Spacing();
        ImGui::PushItemWidth(220.0f * menuResScale);

        ImGui::SeparatorText(UI_CN("性能开销"));

        {
            ...
            if (ImGui::SliderInt(UI_CN("模型遍数"), &passes, 1, passLimit,
                                 passes == 1 ? UI_CN("%d（原生）") : UI_CN("%dx 模型开销")))
            ...
            if (bool lift = unlocked; ImGui::Checkbox(UI_CN("解除遍数限制"), &lift))
            {
            ...
            if (unlocked)
            {
                ...
                if (live > (int) DlssNr::kDefaultMaxPasses)
                    ImGui::TextColored(ImVec4(1.00f, 0.25f, 0.85f, 1.0f),
                                       UI_CN("%d 遍：每帧约为 %dx 模型开销。"), live, live);
                else
                    ImGui::TextColored(ImVec4(0.95f, 0.70f, 0.20f, 1.0f),
                                       UI_CN("已解除限制。继续增加一遍，就会多运行一次完整模型。"));
            }
            ...
            if (liveCount > 1)
            {
                if (ImGui::TreeNode(UI_CN("分遍设置")))
                {
                    ...
                    {
                        const std::string label = UiUtf8(L"第 ") + std::to_string(pass + 1) + UiUtf8(L" 遍");
                        ...
                        edited |= PassOverrideSlider(UI_CN("强度"), &own.Intensity,
                                                     config->DlssNrIntensity.value_or_default(),
                                                     0.0f, 4.0f, pass);
                        edited |= PassOverrideSlider(UI_CN("细节强度"), &own.LocalStructure,
                                                     config->DlssNrLocalStructure.value_or_default(),
                                                     0.0f, 4.0f, pass);
                        edited |= PassOverrideSlider(UI_CN("局部色调"), &own.LocalTone,
                                                     config->DlssNrLocalTone.value_or_default(),
                                                     0.0f, 4.0f, pass);
                        edited |= PassOverrideSlider(UI_CN("皮肤结构"), &own.SkinStructure,
        ...
        if (ImGui::SliderInt(UI_CN("模型分辨率"), &scalePercent, 25, 200, "%d%%"))
...

        {
            ...
            if (dual)
            {
                ...
                static const std::string enlargerSpatial = UiUtf8(L"空间放大（无需运动矢量）");
                ...
                if (ImGui::Combo(UI_CN("放大方式"), &index, enlargerNames, IM_ARRAYSIZE(enlargerNames)))
                {
            ...
            if (ImGui::Checkbox(UI_CN("在超分前运行"), &preUpscale))
            ...
            ImGui::SameLine();
            ImGui::TextColored(ImVec4(0.95f, 0.70f, 0.20f, 1.0f), UI_CN("（实验性）"));

            HelpMarker(UiUtf8(L"让模型处理“即将送入超分器的渲染帧”，而不是超分器已经输出的结果。模型直接在渲染分辨率运行，因此在 Performance 模式下，开销大约只有超分后运行的 1/4，而且看到的是原始渲染像素而不是重建像素。\n\n它与降低“模型分辨率”不同：这里模型是在一张真实的小尺寸帧上 1:1 运行，之后由超分器把模型处理结果和其他内容一起放大，因此不会因为模型内部再缩放而额外变软。\n\n这仍属于实验性路径。此阶段的颜色每帧带有不同的亚像素抖动，而模型并不知道该偏移，因此历史重投影可能出现错误。移动镜头时要重点观察细节闪烁、游动或不稳定。").c_str());
        }
        ...
        // picture and the two modes are identical, so the control says so by going grey.
        {
            ...
            static const std::string enlargeClassic = UiUtf8(L"经典");
            static const std::string enlargeResidual = UiUtf8(L"匹配残差");
            ...
            if (ImGui::Combo(UI_CN("放大合成"), &enlarge, enlargeNames, IM_ARRAYSIZE(enlargeNames)))
                config->DlssNrTransfer = (uint32_t) enlarge;

            if (!reduced)
                ImGui::EndDisabled();

            HelpMarker(UiUtf8(L"当模型分辨率低于当前帧时，决定如何把模型结果重新合成回完整尺寸。\n\n“经典”会直接把模型的小尺寸结果与完整帧合成。两者之间既包含模型真正做出的修改，也包含缩小/放大带来的模糊差异，合成阶段无法区分它们；模型分辨率越低，这种误差越明显，50% 时尤其容易表现为色偏。\n\n“匹配残差”只提取并放大模型真正产生的差值，再叠加到完整尺寸的代理图上。这样参与比较的两张图都处于完整分辨率，从小尺寸栅格带回来的只有模型修改本身。\n\n在 100% 模型分辨率下两种方式完全等价，因此不会产生差异。该方案源自本分支中 hhkbble 的多遍处理工作。").c_str());
        }

        ImGui::SeparatorText(UI_CN("效果混合"));

        float transfer = config->DlssNrTransferStrength.value_or_default();
        if (ImGui::SliderFloat(UI_CN("细节强度"), &transfer, 0.0f, 2.0f, "%.2f"))
            config->DlssNrTransferStrength = transfer;

        HelpMarker(UiUtf8(L"控制最终画面向模型结果混合多少。模型输出不是简单“加”到原画面上，而是一张完整图像；系统会先按原始画面校正其亮度，再在两张完整图像之间插值。\n\n0 = 完全保留超分器原始输出；1 = 完全采用模型结果。\n\n高于 1 会沿同一方向继续外推，这并不是模型原生设计用途，适合临时观察模型到底改变了什么，确认后通常应调回较低值。如果只是想让神经渲染更明显，优先提高这里的“细节强度”；“强度”是模型内部参数，模型会自行决定如何响应。").c_str());

        float colour = config->DlssNrColourStrength.value_or_default();
        if (ImGui::SliderFloat(UI_CN("色彩强度"), &colour, 0.0f, 1.0f, "%.2f"))
            config->DlssNrColourStrength = colour;

        HelpMarker(UiUtf8(L"控制模型的“颜色变化”是否随亮度与细节一起进入最终画面。\n\n0 会完全保留游戏原本的色相，只让亮度/细节体现模型结果；1 则同时采用模型输出的颜色，并限制在 AP1 可表示范围内。\n\n该过程是在两张完整图像之间插值，而不是把颜色差值直接叠加到原图，因此本身不会无约束地推动色相，能避免早期实现中暖色物体被推成绿色之类的问题。").c_str());

        ImGui::SeparatorText(UI_CN("模型设置"));

        ImGui::TextUnformatted(UI_CN("这些参数会在模型构建时读取；修改后会在片刻后重新构建模型。"));

        static const std::string nrPresetDefault = UiUtf8(L"默认");
        static const std::string nrPreset1 = UiUtf8(L"预设 1");
        static const std::string nrPreset2 = UiUtf8(L"预设 2");
        static const std::string nrPreset3 = UiUtf8(L"预设 3");
        ...
        if (ImGui::Combo(UI_CN("模型预设"), &preset, nrPresetNames, IM_ARRAYSIZE(nrPresetNames)))
            config->DlssNrPreset = (uint32_t) preset;

        HelpMarker(UiUtf8(L"“默认”表示让模型自行选择。\n\n这里的预设编号与 DLSS 超分或光线重建的预设不是同一套含义；相同数字在这里代表的是不同模型设置。").c_str());

        static const std::string nrStyleDefault = UiUtf8(L"默认（标准）");
        static const std::string nrStyleNatural = UiUtf8(L"自然");
        static const std::string nrStyleCinematic = UiUtf8(L"电影感");
        ...
        if (ImGui::Combo(UI_CN("风格"), &style, nrStyleNames, IM_ARRAYSIZE(nrStyleNames)))
            config->DlssNrStyle = (uint32_t) style;

        HelpMarker(UiUtf8(L"模型自身提供的处理风格。\n\n默认（标准）：效果最强，会提升局部对比、压深光照，也更容易出现过饱和或明显的风格化；很多“模型改变了游戏原本观感”的现象都来自这一档。\n\n自然：保留同类细节增强，但处理更克制，肤色与整体明暗关系更接近游戏原始渲染。\n\n电影感：减少高光发亮和过度处理，画面更偏电影式质感。\n\n这些参数在模型构建时读取，修改后会触发重新构建。名称来自社区测试，NVIDIA 的二进制中并未提供官方名称。").c_str());

        DeferredSlider(UI_CN("强度"), &config->DlssNrIntensity, 0.0f, 2.0f);

        HelpMarker(UiUtf8(L"模型内部自身的强度参数。它与上方“细节强度”不同：这里会改变模型内部处理，而“细节强度”是在模型输出之后再缩放最终效果。").c_str());

        DeferredSlider(UI_CN("局部结构"), &config->DlssNrLocalStructure, 0.0f, 2.0f);

        DeferredSlider(UI_CN("局部色调"), &config->DlssNrLocalTone, 0.0f, 2.0f);


        DeferredSlider(UI_CN("皮肤结构"), &config->DlssNrSkinStructure, -1.0f, 2.0f);

        HelpMarker(UiUtf8(L"-1 表示跟随“局部结构”，也是模型自身的默认行为，并不等于强度为 0。设置为 0 或更高数值后，皮肤结构会独立于画面其他区域进行控制。").c_str());

        bool autoMask = config->DlssNrAutoMask.value_or_default();
        if (ImGui::Checkbox(UI_CN("自动皮肤遮罩"), &autoMask))
            config->DlssNrAutoMask = autoMask;

        HelpMarker(UiUtf8(L"允许模型自动识别皮肤区域，而不是对整张画面一视同仁地处理。").c_str());

        ImGui::SeparatorText(UI_CN("色彩与曝光"));

        ImGui::TextDisabled(UiUtf8(L"模型训练时使用的是已经完成色调映射、采用 sRGB 编码的最终画面；而超分器输出通常是线性且没有固定上限的 HDR 数据。下面这些设置决定如何把它映射成模型能够正确识别的范围。若游戏报告该帧已经完成色调映射，则会直接跳过这套转换。").c_str());
        ...
        // choice is made on what is available rather than on what sounds best.
        {
            ...
            static const std::string sourcePaperWhite = UiUtf8(L"仅使用纸白");
            static const std::string sourceGameExposure = UiUtf8(L"使用游戏自身曝光");
            static const std::string sourceScanBuffer = UiUtf8(L"使用扫描找到的缓冲区");
            ...
            if (ImGui::Combo(UI_CN("白点来源"), &source, sourceNames, IM_ARRAYSIZE(sourceNames)))
            {
            ...
            else if (source == 2)
            {
                ...
                if (anchorNow <= 0.0f)
                {
                    ...
                    if (watching == 0)
                        ImGui::TextColored(ImVec4(0.9f, 0.6f, 0.25f, 1.0f),
                                           UI_CN("未发现形态符合曝光数据的缓冲区。"));
                    else
                        ImGui::TextColored(ImVec4(0.9f, 0.6f, 0.25f, 1.0f),
                                           UI_CN("正在监测 %u 个候选项，目前都没有变化——请在明暗环境之间移动。"),
                                           watching);
                }
                else if (!haveAnchor)
                    ImGui::TextColored(ImVec4(0.9f, 0.6f, 0.25f, 1.0f),
                                       UI_CN("已找到候选项。先调节下方纸白直到画面正常，再点击“在此标定”。"));
                else
                {
                    ...
                    ImGui::TextColored(ImVec4(0.45f, 0.8f, 0.45f, 1.0f),
                                       UI_CN("已标定：扫描值 %.5f  ->  白点 %.2f"), anchorNow, w);
                }
            }
            else if (haveExposure)
            {
                ImGui::TextColored(ImVec4(0.45f, 0.8f, 0.45f, 1.0f),
                                   UI_CN("该游戏提供了曝光数据——上方对应选项可以直接使用它。"));
            }
        }
        ...
        if (wpSource == 2)
        {
            ...
            const std::string lbl = editingRow
                                        ? UiUtf8(L"纸白（正在编辑标定点 ") + std::to_string(selectedAnchor + 1) + UiUtf8(L"）")
                                        : UiUtf8(L"纸白");
            ...
            if (ImGui::SliderFloat(UI_CN("扫描结果微调"), &trim, 0.25f, 4.0f, "%.2fx",
                                   ImGuiSliderFlags_Logarithmic))
                config->DlssNrScanTrim = std::clamp(trim, 0.25f, 4.0f);

            ImGui::SameLine();

            if (ImGui::SmallButton(UI_CN("重置##scantrim")))
        ...
        else if (wpSource == 1)
        {
            ...
            const std::string trimLabel =
                ofScan ? UiUtf8(L"扫描结果微调") : UiUtf8(L"游戏曝光微调");
            ...
            if (ImGui::SmallButton(UI_CN("重置##wptrim")))
            {
                if (ofScan)
                    config->DlssNrScanTrim = 1.0f;
                else
            ...
            if (DlssNr::ExposureScan::Scanning())
            {
                ...
                if (!anchors.empty())
                {
                    ...
                    for (size_t i = 0; i < anchors.size(); ++i)
                    {
                        ...
                        char row[160];
                        const std::string rowFormat = UiUtf8(L"%s 扫描 %.4f  ->  白点 %.2f%s");
                        const std::string editingText = sel ? UiUtf8(L"   [编辑中]") : std::string();
                    ...
                    ImGui::TextDisabled(UI_CN("点击某一行可用上方滑块编辑该标定点；再次点击即可取消选择并控制实时点。> 表示当前正在使用的标定点。"));
                }
                ...
                if (anchors.size() == 1)
                {
                    ...
                    if (ImGui::Checkbox(UI_CN("数值变化方向相反"), &inverted))
                        config->DlssNrScanInverted = inverted;

                    HelpMarker(UiUtf8(L"如果环境越亮，画面反而朝错误方向变化，请勾选此项。多数引擎存储的曝光值会随场景变亮而下降，也有引擎存储其倒数；仅凭缓冲区形态无法判断方向。添加第二个不同光照下的标定点后，系统可以由数据自动确定方向，因此该选项会不再需要。").c_str());
                }
                ...
                if (isSource && live > 0.0f && !anchors.empty())
                {
                    ...
                    ImGui::TextColored(ImVec4(0.45f, 0.8f, 0.45f, 1.0f),
                                       UI_CN("扫描 %.5f  ->  白点 %.2f   （%u 个标定点）"), live, w,
                ...
                if (ImGui::TreeNode(UI_CN("高级信息")))
                {
                    ...
                    else
                    {
                        for (size_t i = 0; i < found.size(); ++i)
                        {
                            ...
                            if (c.reads == 0)
                            {
                                ImGui::TextDisabled(UI_CN("%zu. %s -- 尚未读取"), i + 1, c.shape.c_str());
                                continue;
                            }
                            ...
                            ImGui::TextColored(c.moves ? ImVec4(0.45f, 0.8f, 0.45f, 1.0f)
                                               ...
                                               UI_CN("%zu. %s = %.5f  （范围 %.5f..%.5f） %s"), i + 1,
                                               c.shape.c_str(), c.latest, c.lowest, c.highest,
                                               c.moves ? UI_CN("变化中") : UI_CN("暂时不变"));
                        }

                        ImGui::TextDisabled(UI_CN("从阴影走到明亮区域；真正的曝光值应该会变化。"));
                        ImGui::TextDisabled(UI_CN("如果数值只会单向持续上升，那通常是计数器，而不是曝光。"));
                    }
        ...
        const std::string holdButton =
            holding ? UiUtf8(L"恢复") : holdPending ? UiUtf8(L"取消锁定") : UiUtf8(L"锁定帧");
        ...
        ImGui::EndDisabled();
        HelpMarker(UiUtf8(L"仅用于 D3D12 检查：锁定下一帧成功处理后的代理图、模型输出和原始画面，便于静态比较。锁定期间“对比”和“调试视图”仍可交互，但模型不会继续运行，直到点击“恢复”。这不会暂停游戏本身。\n\n模型参数和色彩参数的修改会在恢复后生效；改变分辨率会自动解除锁定。执行“捕获”时会先恢复实时渲染，而“锁定帧”会等待当前捕获完成。\n\n原生 Vulkan 或实验性代理路径下不可用。").c_str());
        ImGui::SameLine();

        if (capturing)
        {
            ImGui::TextDisabled(UI_CN("正在捕获……"));
        }
        else if (ImGui::Button(UI_CN("捕获 8 帧")))
        {
            DlssNr::RequestCapture(8);
        }

        HelpMarker(UiUtf8(L"连续捕获 8 帧，并分别保存两套结果：一套是超分器原始输出，另一套是应用神经渲染后的画面。\n\n两套图来自同一次运行、同一批帧，唯一变量就是神经渲染本身，因此比录制两段不同视频更适合做严谨对比；视频录制不仅镜头路径难以完全一致，编码器还会丢失细微的时间域信息。\n\n以原始数据写入 OptiScaler 旁的 dlssnr-capture 文件夹。每次固定保存 8 帧，并覆盖上一轮捕获。").c_str());

        static const std::string compareOff = UiUtf8(L"关闭");
        static const std::string compareSide = UiUtf8(L"并排对比");
        static const std::string compareWipe = UiUtf8(L"分割线对比");
